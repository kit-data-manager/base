 ...
 * @author dx6468
 */
public class UserGroupSearch extends HorizontalLayout {

    private static final Logger LOGGER
            = LoggerFactory.getLogger(UserGroupSearch.class);
    public final static String DEBUG_ID_PREFIX
            = UserGroupSearch.class.getName() + "_";
    ...
    public UserGroupSearch(UserGroupAdministrationTab userGroupAdministrationTab) {
        this.userGroupAdministrationTab = userGroupAdministrationTab;
        ...
        view.addPopupVisibilityListener(new PopupView.PopupVisibilityListener() {
            ...
            public void popupVisibilityChange(PopupView.PopupVisibilityEvent event) {
                if (!event.isPopupVisible()) {
                    Table table = userGroupAdministrationTab.getUserGroupTablePanel()
                            .getUserGroupTable();
                    table.select(table.firstItemId());
                    getColumnBox().select(null);
                    getFilterOptions().select(UserGroupFilter.SearchSpace.STARTS);
                }
            }
        });
    }
    ...
    private void buildFilterButton() {
        ...
        filterButton.addClickListener(new Button.ClickListener() {
            ...
            public void buttonClick(Button.ClickEvent event) {
                ...
                UserGroupContainer.Property property = (UserGroupContainer.Property) getColumnBox().getValue();
                UserGroupFilter.SearchSpace searchSpace = (UserGroupFilter.SearchSpace) getFilterOptions().getValue();
                userGroupAdministrationTab.getUserGroupTablePanel().addTableFilter(filterExpression, property, searchSpace);
    ...
    private void reloadColumnBox() {
        ...
        getColumnBox().addItems(Arrays.asList(UserGroupContainer.Property.values()));

        for (UserGroupContainer.Property property : UserGroupContainer.Property.values()) {
            getColumnBox().setItemCaption(property, property.propertyId);
        }
    }
    ...
    private void buildRemoveAllFiltersButton() {
        ...
        removeAllFiltersButton.addClickListener(new Button.ClickListener() {
            ...
            @Override
            public void buttonClick(Button.ClickEvent event) {
                userGroupAdministrationTab.getUserGroupTablePanel().removeAllTableFilters();
            }
        });
    }
}
