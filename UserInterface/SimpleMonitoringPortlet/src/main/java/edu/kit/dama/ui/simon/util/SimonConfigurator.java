...
public class SimonConfigurator implements ConfigurationListener, FileAlterationListener {
  ...
  SimonConfigurator() {
    ...
    try {
      ...
      LOGGER.warn("Failed to start configuration monitor. Configuration changed won't be tracked.");
    }
  }
  ...
  public void setConfigLocation(File pPath) throws ConfigurationException {
    ...
    try {
      ...
      LOGGER.error("Failed to initialize directory observer. Configuration changes won't be tracked.", e);
      observer = null;
    }
  ...
  private void configure() throws ConfigurationException {
    ...
    for (File f : configPath.listFiles()) {
      ...
      try {
        ...
        LOGGER.error("Failed to read probe configuration from file " + f, ex);
      }
    }
      ...
      try {
        ...
        if (probeClass != null) {
          ...
          if (probe.configure(config)) {
            ...
            LOGGER.warn("Failed to configure probe " + probeName + ". Probe will be unavailable.");
          }
          probes.add(probe);
        } else {
          LOGGER.error("Property probe.class for probe " + probeName + " not defined. Ignoring probe.");
        }
      } catch (ClassNotFoundException | InstantiationException | IllegalAccessException ex) {
        LOGGER.error("Failed to instantiate probe from file " + config.getPath(), ex);
      }
    }
  }
  ...
  public void configurationChanged(ConfigurationEvent ce) {
    try {
      ...
      LOGGER.error("Failed to reconfigure Simon", ex);
    }
  }
  ...
  public void onDirectoryDelete(File file) {
    try {
      ...
      LOGGER.error("Failed to recheck configurations in 'onDirectoryDelete' event", ex);
    }
  }
  ...
  public void onFileCreate(File file) {
    ...
    try {
      ...
      LOGGER.error("Failed to recheck configurations in 'onFileCreate' event for file " + file, ex);
    }
  }
  ...
  public void onFileChange(File file) {
    ...
    try {
      ...
      LOGGER.error("Failed to recheck configurations in 'onFileChange' event for file " + file, ex);
    }
  }
  ...
  public void onFileDelete(File file) {
    ...
    try {
      ...
      LOGGER.error("Failed to recheck configurations in 'onFileDelete' event for file " + file, ex);
    }
  }
