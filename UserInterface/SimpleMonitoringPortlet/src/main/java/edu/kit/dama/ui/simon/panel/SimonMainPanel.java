...
public class SimonMainPanel extends CustomComponent implements Button.ClickListener {
  ...
  private void reload() {
    ...
    tabs.add(mainLayout.addTab(tabsByCategory.get("Overview"), "Overview"));

    for (String key : aKeys) {
      Tab categoryTab = mainLayout.addTab(tabsByCategory.get(key), key);
  ...
  /**
   * Assign the current "simon says" comment to the label including the correct
   * format.
   *
  ...
  /**
   * Build the overview tab including the list of all categories and der overall
   * status.
   *
  ...
  private Component buildOverviewTab(String[] pCategories) {
    ...
    for (String category : pCategories) {
      ...
      Label name = new Label("<h3>" + category + "</h3>", ContentMode.HTML);
      name.setWidth("200px");
      name.setHeight("40px");
      ...
      for (AbstractProbe probe : probes) {
        ...
        if (first) {
          probeLabel.addStyleName("probe-outer-left");
          first = false;
        }

        lastLabel = probeLabel;
      ...
      if (lastLabel != null) {
        if (probes.size() == 1) {
          lastLabel.addStyleName("probe-single-probe");
        } else {
          lastLabel.addStyleName("probe-outer-right");
        }
      }

      rowLayout.setWidth("700px");
      rowLayout.setHeight("40px");
