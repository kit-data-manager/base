...
public final class RestClientUtils {
  ...
  public static <C> C performGet(Class<C> pEntityClass, WebResource pWebResource, MultivaluedMap pQueryParams) {
    ...
    returnValue = webResource.get(ClientResponse.class);
   ...
   *
   * @param <C> entity class
   * @param pEntityClasses An array of classes needed to deserialize the entity.
   * @param pWebResource instance of webresource.
   * @param pQueryParams url parameters
   * @return client response
   */
  ...
  public static <C> C performPut(Class<C> pEntityClass, WebResource pWebResource,
    ...
    WebResource webResource = prepareWebResource(pWebResource, pQueryParams);
    logFormParams(pFormParams);
    returnValue = webResource.put(ClientResponse.class, pFormParams);
    return createObjectFromStream(pEntityClass, returnValue);
  }
  ...
  public static <C> C performDelete(Class<C> pEntityClass, WebResource pWebResource, MultivaluedMap pQueryParams) {
    ClientResponse returnValue;
    WebResource webResource = prepareWebResource(pWebResource, pQueryParams);
    returnValue = webResource.delete(ClientResponse.class);
    return createObjectFromStream(pEntityClass, returnValue);
  }
   ...
   * deserialization fails for some reason. In some cases, pEntityClass might be
   * 'null', e.g. if no deserializable output is expected. In this cases it is
   * possible, to obtain the response object by setting the return type C to
   * ClientResponse. If this is the case and pEntityClass is null, pResponse
   * will be returned.
   *
   * @param <C> entity class of ClientResponse if pResponse should be returned.
