...
public final class RestUtils {
   ...
   * request and obtains the user id for these two tokens. If a user id could be
   * determined, an authorization context is created together with the provided
   * group id and the max. user role in this group. This call fails if access
   * key and token do not match, if no user id is found for the provided access
   * tokens or if the obtained user is no member of the group with the provided
   * id. In all these cases a WebServiceException is thrown.
   *
   * @param hc The HttpContext used to extract access key and secret.
   * @param pGroupId The group id used to build the authorization context.
   *
   * @return The authorization context including pGroupId and the according max.
   * role of the obtained user.
   */
  public static IAuthorizationContext authorize(HttpContext hc,
          GroupId pGroupId) {
    ...
    try {
      ...
      LOGGER.error("Failed to handle verify OAuth signature", ose);
      throw new WebServiceException(OAUTH_ERROR);
    } catch (Exception ex) {
  ...
  /**
   * Serializes one or more entities of the same type to an XML object graph to
   * a stream which is returned. This method can be returned directly by a REST
   * call. On the client side the deserialization can take place according to
   * the object graph definition.
   *
   * @param pEntityClass An array of classes of supported entities.
   * @param pGraphName The object graph name used to serialize the object. If no
   * graph name is provided the entire object is serialized.
    ...
    return new StreamingOutput() {
      ...
      public void write(OutputStream arg0) {
        try {
          ...
          LOGGER.
                  error("Failed to marshal result to output stream.", e);
   ...
   *
   * To check the access <i>servletRequest.getRemoteAddr()</i> is compared to a
   * semicolon separated list of IPs defined as an init-param named 'allow.from'
   * in the web.xml. The param is accessed via the provided <i>resource</i>
   * argument. If this init-param is not defined, only access from 127.0.0.1
   * will be granted.
   *
   * If the obtained remote address cannot be mapped to any IP in 'allow.from' a
   * WebApplicationException with error code 403 (HTTP.FORBIDDEN) is thrown.
