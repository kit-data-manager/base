...
public class IngestInformationPersistenceTest {
  ...
  public void getEntitiesByNullOwner() {
    Logger.getLogger(IngestInformationPersistenceTest.class.getName()).log(Level.INFO, "Try getting entities with owner 'null'. Expecting IllegalArgumentException.");
    testCandidate.getEntitiesByOwner(null, secCtx);
  }
  ...
  public void updateEntityStatus() {
    ...
    List<IngestInformation> resultList = testCandidate.getEntitiesByStatus(INGEST_STATUS.INGEST_FAILED, null);
    Assert.assertEquals(0, resultList.size());

    IngestInformation randomEntity = entitiesCtx.get(0);
    int affectedRows = testCandidate.updateStatus(randomEntity.getId(), INGEST_STATUS.INGEST_FAILED, "Some Error", null);
   ...
   * fault if both lists: - are not 'null' - have the same size - have the same
   * elements
   */
