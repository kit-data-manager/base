...
public final class StagingService {
   ...
   * file is created inside the staging folder to be able to "undelete" later as
   * long as the physical deletion has not been performed.
   *
  ...
  /**
   * Prepares the ingest for the digital object with the provided digital object
   * ID. This method will be called by the transfer preparation handler if the
   * user requests an ingest. The method takes care, that there is an upload
   * folder existing after this call, which is writeable by the calling user.
   * This folder is stored at the preparation result entity returned by this
   * method, together with the appropriate status. The update of the ingest
   * information entity within the data backend is done by the preparation
   * handler.
   *
   * @param pDigitalObjectId The id of the digital object to ingest.
   * @param pAccessPointId The If of the AccessPoint used for ingest. It defines
   * access protocol and target folder used to upload data.
   ...
   * call to this method is blocking and may take a long time.<br/>
   *
   ...
   * steps are performed:
   * <ul>
   * <li>Obtain the location of the data, depending on the AccessPoint used for
   * the ingest.</li>
    ...
    try {
      ...
      if (localPath == null) {
        ...
        if (SystemUtils.lockFolder(localPath)) {
          ...
          LOGGER.error("Failed to lock ingest folder");
        }
        ...
        if (preProcessingSucceeded) {
          ...
          if (virtualizedFileTree == null) {
            ...
            if (StagingConfigurationManager.getSingleton().getDataOrganizationAdapter().saveFileTree(virtualizedFileTree)) {
              ...
              LOGGER.error("Failed to register data organization.");
              ingest.setStatus(INGEST_STATUS.INGEST_FAILED.getId());
  ...
  /**
   * This method is used internally in an automated fashion to finalize the next
   * ingest. At first, all finalizable ingests are obtained and sorted by their
   * expiration date. Afterwards, for the ingest expiring next {@link #finalizeIngest(edu.kit.dama.commons.types.DigitalObjectId)
   * } is called.
   *
   * @return TRUE if an ingest could be finalized.
   */
...
//</editor-fold>
   ...
   * download folder existing after this call, which is writeable by the calling
   ...
   * download information entity within the data backend is done by the
   * preparation handler.
   *
   ...
   * context is only used for the initial query. Download-related access is done
   * using the context stored in the obtained download entity.
   *
  ...
  /**
   * This method is used internally in an automated fashion to finalize the next
   * ingest. Therefor the ingest service, the storage virtualization and the
   * data organization are needed. At first, all finalizable ingests are
   ...
   * which will expire first. As a result, the virtual file tree is returned and
   * will be stored using the data organization if the ingest has succeeded. If
   * everything is done, the ingest information is update using the ingest
   * information service adapter.
   *
   ...
   * the initial query. Download-related access is done using the context stored
...
//</editor-fold>
   ...
   * initial query. Download-related access is done using the context stored in
   * the obtained download entity.
   *
