...
public final class StagingConfigurationPersistence {
  ...
  public static synchronized StagingConfigurationPersistence getSingleton(String pUnit) {
    if (SINGLETON == null) {
      ...
      if (SINGLETON.getPersistenceUnit() != null && pUnit != null && !SINGLETON.getPersistenceUnit().equals(pUnit)) {
   ...
   *
   * @throws UnauthorizedAccessAttemptException Internal context is not allowed
   * to save. Should never happen.
   */
  public StagingAccessPointConfiguration saveAccessPointConfiguration(StagingAccessPointConfiguration pNewMethod) throws UnauthorizedAccessAttemptException {
    ...
    StagingAccessPointConfiguration result = mdm.save(pNewMethod);
    mdm.close();
    return result;
  }
   ...
   * returned. If more than one AccessPoint was found, the first AccessPoint is
   * returned and a warning is logged as there should be only one AccessPoint
   * per name. By default, only one AccessPoint is obtained and will be
   * returned.
   *
  ...
  public StagingAccessPointConfiguration findAccessPointConfigurationByName(String pName) {
    ...
    IMetaDataManager mdm = getMetaDataManager();
    try {
      result = mdm.findResultList("SELECT m FROM StagingAccessPointConfiguration m WHERE m.name='" + pName + "'", StagingAccessPointConfiguration.class);
    } catch (UnauthorizedAccessAttemptException ex) {
   ...
   * pGroupId is null, AccessPoints with an unspecified groupId are checked. If
   * no default AccessPoint was found, 'null' is returned. If more than one
  ...
  /**
   * Find all AccessPoints for the provided group. If pGroupId is null, an empty
   * string is used.
   *
  ...
  public List<StagingAccessPointConfiguration> findAccessPointConfigurationsForGroup(String pGroupId) {
    ...
    try {
      result = mdm.findResultList("SELECT h FROM StagingAccessPointConfiguration h WHERE h.groupId='" + groupId + "' OR h.groupId IS NULL", StagingAccessPointConfiguration.class);
    } catch (UnauthorizedAccessAttemptException ex) {
  ...
  public StagingAccessPointConfiguration findAccessPointConfigurationByUniqueIdentifier(String pUniqueIdentifier) {
    ...
    IMetaDataManager mdm = getMetaDataManager();
    try {
      result = mdm.findSingleResult("SELECT m FROM StagingAccessPointConfiguration m WHERE m.uniqueIdentifier='" + pUniqueIdentifier + "'", StagingAccessPointConfiguration.class);
    } catch (UnauthorizedAccessAttemptException ex) {
