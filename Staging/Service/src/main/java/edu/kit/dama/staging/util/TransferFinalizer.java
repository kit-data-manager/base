...
public class TransferFinalizer {
    ...
    private boolean performFinalization() {
        ...
        boolean result = true;
        List<IngestInformation> ingests = IngestInformationServiceLocal.getSingleton().getIngestInformationByStatus(INGEST_STATUS.PRE_INGEST_FINISHED.getId(), 0, 20, AuthorizationContext.factorySystemContext());
        System.out.println("FINALIZING " + ingests.size() + " INGESTS");
        for (IngestInformation ingest : ingests) {
            System.out.println("FINALIZE INGEST " + ingest.getId());
            IngestInformationServiceLocal.getSingleton().updateStatus(ingest.getId(), INGEST_STATUS.INGEST_RUNNING.getId(), null, AuthorizationContext.factorySystemContext());
            System.out.println("RESULT: " + StagingService.getSingleton().finalizeIngest(new DigitalObjectId(ingest.getDigitalObjectId()), AuthorizationContext.factorySystemContext()));
        }

        List<DownloadInformation> downloads = DownloadInformationServiceLocal.getSingleton().getDownloadInformationByStatus(DOWNLOAD_STATUS.SCHEDULED.getId(), 0, 20, AuthorizationContext.factorySystemContext());
        System.out.println("FINALIZING " + downloads.size() + " DOWNLOADS");
        for (DownloadInformation download : downloads) {
            System.out.println("FINALIZE DOWNLOAD " + download.getId());
            DownloadInformationServiceLocal.getSingleton().updateStatus(download.getId(), DOWNLOAD_STATUS.PREPARING.getId(), null, AuthorizationContext.factorySystemContext());
            System.out.println("RESULT: " + StagingService.getSingleton().finalizeDownload(download));
        }
...
//        }
     ...
     *
     * @throws ConfigurationException If the command line options were in a
     * wrong format.
