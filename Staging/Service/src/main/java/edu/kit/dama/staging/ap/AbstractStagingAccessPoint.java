...
public abstract class AbstractStagingAccessPoint implements IConfigurable {
     ...
     * check, whether this method can be used or not, e.g. whether the local
     * path is accessible and if all provided settings are valid. Furthermore,
     * this setup is called while instantiating any AccessPoint to perform
     * availability checks and to check the configuration.
     *
     * @param pCheckOnly Only perform checks without changing anything in the
     * underlaying system, e.g. creating/deleting folders.
     *
    ...
    public final void setup(boolean pCheckOnly) throws ConfigurationException {
        ...
        edu.kit.dama.util.FileUtils.isAccessible(new File(localBasePath), 1000);
    ...
    /**
     * Perform the actual preparation for the provided transfer information
     * entity using a map of properties and a security context.In a first
     * preparation phase,
   * {@link #prepareCache(edu.kit.dama.staging.interfaces.ITransferInformation, edu.kit.dama.authorization.entities.IAuthorizationContext) }
     * is used to perform the basic, AccessPoint-specific preparation of the
     * base caching location. Afterwards, the common KIT Data Manager specific
     * staging structure is created by this abstract class.
     *
     ...
     *
     * @return TRUE if the cleanup was succeeded (.deleted was created) or no
     * files were left, FALS if .deleted could not be created and there are
     * files left.
     */
   ...
   * {@link #getAccessUrl(edu.kit.dama.staging.interfaces.ITransferInformation, edu.kit.dama.authorization.entities.IAuthorizationContext) }
     * afterwards and this location should then be accessible using a
     * appropriate data transfer client. This method is executed while calling
     * prepare(), direct access should be forbidden.
     *
    ...
    /**
     * Returns the URL for the data folder. In order to be able to obtain a
     * valid data URL, pBaseFolderUrl should be the result of calling {@link #getAccessUrl(edu.kit.dama.staging.interfaces.ITransferInformation, edu.kit.dama.authorization.entities.IAuthorizationContext)
