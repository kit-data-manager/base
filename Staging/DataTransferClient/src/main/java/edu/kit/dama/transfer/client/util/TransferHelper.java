...
public final class TransferHelper {
   ...
   * checkpoint. This method only checks if a checkpoint file is located within
   * the transfer temp directory and returns TRUE if this file exists, no matter
   ...
   * @param pContext The credentials to access the staging service used to check
  ...
  public static boolean createCheckpoint(TransferTaskContainer pContainer) {
    ...
    try {
      ...
      LOGGER.warn("Failed to create checkpoint. I'll proceed, but will not be able to resume unfinished transfers.", ioe);
    } finally {
      if (fout != null) {
        try {
          fout.close();
  ...
  /**
   * Factory a new transfer client for the provided transfer task container.
   * This method is intended to be used for downloads. The download destination
   * will be set in the transfer task container.
   *
   * @param pContainer The transfer task container.
