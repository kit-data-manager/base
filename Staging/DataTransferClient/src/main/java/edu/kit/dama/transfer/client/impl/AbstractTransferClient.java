...
public abstract class AbstractTransferClient extends Thread implements ITransferStatusListener, ITransferTaskListener {
     ...
     * contain any structural information. In this case, the bit stream from
     * file KEY is just copied to file VALUE.
     */
    ...
    /**
     * The delay until the actual transfer starts. This flag was only
     ...
     * number of client instances is defined. However, normally a single
     * protocol client can be used by several (&gt;&gt;5) transfer tasks in
     * parallel).
     */
     ...
     * pContainer defines an ingest container, an IllegalArgumentException will
     * be thrown.
     */
    ...
    /**
     * Default constructor to create a transfer client for ingests. In this
     * case, the destination is obtained by the transfer task container.
     *
     * @param pContainer The transfer task container.
     */
    ...
    /**
     * Set the transfer task container. This method is intended to be used only
     * in case a transfer is restored. Otherwisem, it is recommended to load the
    ...
    /**
     * Perform staging processors. This may include preparation of the data
     * which has to be transfered. Available processors and their selection is
     * done in beforehand by the user via a user interface.
     *
     ...
     *
     * @throws PrepareTransferException If there was an internal error that
     * forced the preparation to stop.
     */
    ...
    /**
     * This method is called by the ShutdownHook attached to each transfer
     * client. If the transfer client is terminated by the user via SIGINT
     * (text-based client) or by closing the main application window (graphical
     * client), the ShutdownHook will be executed and calls performShutdown, to
     * allow the implementation to react on the shutdown request, e.g., by
     * cleaning up or by storing the current transfer state to be able to
     * continue at this point later.
     */
    public abstract void performShutdown();

    /**
     * Initialized and perform the actual transfer. This method should return
     * TRUE if the transfer was finished successfully, FALSE if the transfer was
     * interrupted externally (e.g. by a cancel-request from a user) and it
     * should throw a TransferException if there was an internal error that
     * forced the transfer to stop.
     *
     * @return TRUE on success, FALSE on external interruption.
     */
    ...
    /**
     * Check whether this transfer is locked or not. Therefor we'll check if
     * there is a file '.lock' located in the transfer's temporary directory.
     * Each transfer is locked as part of the internal preparation. If the
    ...
    /**
     * Create the temporary transfer directory and a .lock file to avoid
     * multiple transfers for the same DOID. Calling this method is part of the
     ...
     * temporary directory. This method is used only if the transfer has failed
     * or if it was canceled by the user to allow resuming the transfer.
     *
     ...
     *
     * @throws IOException If the temp directory could not be obtained, created
     * or locked.
     */
    private void checkTempDir() throws IOException {
        ...
        if (tempDir.exists()) {
            ...
            while (isTransferLocked()) {
                LOGGER.info("Transfer {} with temp dir {} seems to be locked. Retry in one second.", transferContainer.getTransferInformation().getTransferId(), transferTempDir);
                try {
                    Thread.sleep(1000);
            ...
            if (checkCount == 10) {
                LOGGER.error("Transfer {} with temp dir {} is still locked. Transfer not possible.", transferContainer.getTransferInformation().getTransferId(), transferTempDir);
    ...
    /**
     * Returns the current list of transfer tasks. This list may change over
     * time, as local pre-processing can combine, add or remove entries.
     *
     ...
     * performShutdown() is called to allow implementations of this abstract
     * class to perform custom shutdown steps. The ShutdownHook is only executed
     * when terminating the transfer client normally. For graphical applications
     * this would be if the main windows has closed, for command line
     * applications interrupting the execution via CTRL+C will work. Internally
     * System.exit() would terminate the current VM and execute all shutdown
     * hooks. Nevertheless, stopTransfer() should be used by default to allow a
     * proper cleanup.
     */
    ...
    /**
     * Returns the current transfer status. During transfer, the status should
     * be TRANSFER_STATUS.RUNNING
     *
     * If the transfer has finished, the status is either
     * TRANSFER_STATUS.SUCCEEDED or or it represents the phase, where the
     ...
     *
     * @return TRUE if this phase was finished successfully, FALSE if there was
     * an error or the use canceled the transfer.
     */
    public final boolean prepare() {
        ...
        try {
            ...
            LOGGER.error("Checking temp directory failed.", ex);
    ...
    /**
     * Performs all transfer tasks defined for this client. If the transfer
     * fails, there are MAX_TRIES retries. After this amount, FALSE is returned,
     * the last file is put back to the transfer map and no more files will be
     * transfered.
     *
    ...
    /**
     * Perform cleanup operations in case of a successful transfer. During
     * cleanup the temporary transfer directory will be removed. If the removal
     * fails, the deletion will be sheduled to be performed during exit.
     */
    ...
    /**
     * Try to set the current status. This method only changes the status, if
     * the current status is TRANSFER_STATUS.RUNNING. If the status can be
     * changed, it is checked, whether the transfer was canceled or not. If it
     * was canceled, the status is set to TRANSFER_STATUS.CANCELED, otherwise it
     * is set to pStatus.
     *
    ...
    /**
     * Notifies all transfer task listeners that TransferTask pTask has
     * finished.
     *
     * @param pTask The transfer task.
     */
    ...
    /**
     * ShutdownHook implementation calling performShutdownInternal() if a
     * shutdown of the current VM was detected.
     */
