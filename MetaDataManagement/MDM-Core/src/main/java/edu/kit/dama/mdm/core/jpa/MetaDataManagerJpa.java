...
public class MetaDataManagerJpa implements IMetaDataManager {
  ...
  public final <T> T find(final Class<T> entityClass, final Object primaryKey) throws UnauthorizedAccessAttemptException {
    ...
    try {
      ...
      //-javaagent:D:/.m2/repository2/org/eclipse/persistence/eclipselink/2.5.1/eclipselink-2.5.1.jar 
...
//
    ...
    if (entityManager.contains(entityManaged)) {
      try {
        entityManager.refresh(entityManaged);
      } catch (RuntimeException re) {
...
//  }
   ...
   * <li>Clear the cache of the EntityManager (make all entities unmanaged)</li>
   * </ul>
   *
   * If no transaction is provided (after an operation which did not affect the
   * data backend, e.g. find()), only the cache is cleared.
   *
...
//  }
   ...
   * @param entity The entity.
   *
   * @return TRUE if the Id field could be set to null or if the provided entity
   * is not a JPA entity.
   *
   * @throws UnauthorizedAccessAttemptException If the Id field cannot be
   * accessed.
