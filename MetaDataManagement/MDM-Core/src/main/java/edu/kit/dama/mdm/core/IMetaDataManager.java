...
public interface IMetaDataManager {
   ...
   * 'true' is returned, <i>object</i> can be obtained via methods like find().
   * If 'false' is returned, <i>object</i> has to be persisted first.
   *
   * This method does not change <i>object</i> and won't try to achieve a status
   * where 'true' is returned, e.g. by persisting <i>object</i> implicitly.
   *
  ...
  /**
   * Find instance by primary key. For backends using relational databases (e.g.
   * JPA), the primary key could be an instance of 'Long' stored in a field
   * annotated by <i>@Id</i>.
   *
   * @param <T> any class supported by the chosen model
   ...
   * However, there are <b>no rules</b> on how detailed the result should be. It
   * is <i>recommended</i> to cover at least fields of primitive types, which
   * are directly located within class T. If T representes a complex object
   * containing child elements, the implementation of IMetaDataManager may or
   * may not include these childrens into the search.
   *
   ...
   * Therefore, none of these arguments must be written to or should be checked
   * against the backend.
   *
   * Implementation specific runtime exceptions which may occur should be logged
   * as warning.
   *
   * @param <T> any class supported by the chosen model
   ...
   * should be guaranteed that parameters are handled properly, e.g. by escaping
   * them to avoid SQL-injections or similar vulnerabilities. The paramters are
   * added to the query by providing variables according to their position in
   * the parameters array beginning with 1. Therefore, the first parameter in
   * the parameter array replaces the variable named <b>?1</b> in the query.
   *
   * Implementation specific runtime exceptions which may occur should be logged
   * as warning.
   *
   ...
   * @param queryString The plain query string
   * @param entityClass The entity class
   * @param pFirstIdx The first entity index withing the result list. This index
   * must be >= 0. Values smaller than 0 should be set 0. The result for the
   * case, where <i>pFirstIdx</i> is larger than the max. amount of entries is
   * not defined!
   * @param pResultCount The maximum number of results returned by one call.
   * This value should be >= 1. If the value is smaller than 1, the value has to
   * be <b>ignored</b> by the implementation.
   *
   ...
   * provided entity class. If the query is correct, this method should succeed.
   * If no result was found, <i>null</i> should be returned.
   *
   * Implementation specific runtime exceptions which may occur should be logged
   * as warning.
   *
   ...
   * Performs a native query which is expected to return a single entity of the
   * provided entity class. If the query is correct, this method should succeed.
   * If no result was found, <i>null</i> should be returned. In contrast to {@link #findSingleResult(java.lang.String, java.lang.Class)
   * } this method is intended to be used if the query contains parameters. It
   * should be guaranteed that parameters are handled properly, e.g. by escaping
   * them to avoid SQL-injections or similar vulnerabilities. The paramters are
   * added to the query by providing variables according to their position in
   ...
   * 'instanceof' check or by an appropriate typecast. This method is intended
   * to be used for queries returning no entity, e.g. a count query in JPQL/SQL,
   * but should be applicable also for queries for entities.
   *
   * Implementation specific runtime exceptions which may occur should be logged
   * as warning.
   *
   ...
   * 'instanceof' check or by an appropriate typecast. This method is intended
   * to be used for queries returning no entity, e.g. a count query in JPQL/SQL,
   * but should be applicable also for queries for entities. In contrast to {@link #findSingleResult(java.lang.String)
   * } this method is intended to be used if the query contains parameters. It
   * should be guaranteed that parameters are handled properly, e.g. by escaping
   * them to avoid SQL-injections or similar vulnerabilities. The paramters are
   * added to the query by providing variables according to their position in
   ...
   * Performs a native query which is expected to return a list of entities. The
   * type of the returned entities is undefined and has to be determined later
   * by 'instanceof' check or by an appropriate typecast. In contrast to {@link #findResultList(java.lang.String)
   * } this method is intended to be used if the query contains parameters. It
   * should be guaranteed that parameters are handled properly, e.g. by escaping
   * them to avoid SQL-injections or similar vulnerabilities. The paramters are
   * added to the query by providing variables according to their position in
   ...
   * be <b>ignored</b> by the implementation.
   *
   * @return The result list with the size pResultCount or an empty list if not
   * entity fits the query.
   *
   * @throws UnauthorizedAccessAttemptException Access not allowed for this
   * context.
   */
   ...
   * type of the returned entities is undefined and has to be determined later
   * by 'instanceof' check or by an appropriate typecast. The result list has a
   * fixed maximum size defined by pFirstIdx and pResultCount. In contrast to {@link #findResultList(java.lang.String, int, int)
   * } this method is intended to be used if the query contains parameters. It
   * should be guaranteed that parameters are handled properly, e.g. by escaping
   * them to avoid SQL-injections or similar vulnerabilities. The paramters are
   * added to the query by providing variables according to their position in
   ...
   * entities. This method is intended to be used for high-performance updates
   * on single fields of an entity. Compared to
   ...
   * huge performance advantage, especially for very big object graphs with many
   * entities.
   *
   * Implementation specific runtime exceptions which may occur should be logged
   * as warning.
   ...
   * entire object graph but will directly modify the entity. This leads to a
   * huge performance advantage, especially for very big object graphs with many
   * entities. In contrast to {@link #performUpdate(java.lang.String)
   * } this method is intended to be used if the update contains parameters. It
   * should be guaranteed that parameters are handled properly, e.g. by escaping
   * them to avoid SQL-injections or similar vulnerabilities. The paramters are
   * added to the update query by providing variables according to their
   * position in the parameters array beginning with 1. Therefore, the first
   * parameter in the parameter array replaces the variable named <b>?1</b> in
   * the query.
   *
   * Implementation specific runtime exceptions which may occur should be logged
   * as warning.
   ...
   * the first time (perform an initial persist) or for saving changed entities
   * which were already persisted. Therefore, this method should check if the
   * entity is known, first, e.g. via <i>contains(entity)</i>. If this method
   * returns an entity, this should be the representation of the entity at the
   * backend. If the operation fails, a RuntimeException should occur.
   *
   ...
   * (e.g. a unique id is assigned by JPA) or its reference changes. Therefore,
   * the argument <i>entity</i> should not be used after this method call
   * anymore. Instead, the returned entity is the actual, managed entity which
   * should be used for subsequent accesses.
   *
  ...
  /**
   * Persist the provided entity. This method is intended to be used for initial
   * persist operations. It has to check, wheather the entity already exists or
   * not. If it exists, an exception will be thrown. If not, the new entity will
   * be persisted. If the operation fails, a RuntimeException should occur.
   *
   * During the persist-process the entity may be changed by the implementation
   * (e.g. a unique id is assigned by JPA) or its reference changes. Therefore,
   * the argument <i>entity</i> should not be used after this method call
   * anymore. Instead, the returned entity is the actual, managed entity which
   * should be used for subsequent accesses.
   ...
   * entity. If the entity does not exist (contains(entity) returns 'false'), an
   ...
   * or its reference changes. Therefore, the argument <i>entity</i> should not
   * be used after this method call anymore. Instead, the returned entity is the
   * actual, managed entity which should be used for subsequent accesses.
   *
   * @param <T> any class supported by the chosen model Make a persist or
   * update.
   ...
   *
   * This only works for entities which were persisted before. For new entities,
