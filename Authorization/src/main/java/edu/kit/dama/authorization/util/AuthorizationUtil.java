...
public final class AuthorizationUtil {
    ...
    /**
     * Helper method to determine whether the provided authorization context is
     * an administrator context or not. This is the case if the group id is
     * equal to Constants.SYSTEM_GROUP and/or the role is Role.ADMINISTRATOR.
     *
    ...
    /**
     * Obtain an authorization context for the provided user with groupId
     * 'USERS'. Inside, the GroupServiceLocal is used to obtain the role
     * restriction for building the final context. For querying the GroupService
     * the system user context is used.
     *
     * If no result was found, a context with Role NO_ACCESS is returned.
     *
     * @param pUserId The user id of the resulting context.
     *
     * @return The AuthorizationContext.
     *
     * @throws AuthorizationException If obtaining the authorization context
     * fails.
     */
    public static IAuthorizationContext getAuthorizationContext(UserId pUserId) throws AuthorizationException {
        return getAuthorizationContext(pUserId, new GroupId(Constants.USERS_GROUP_ID));
    }
     ...
     * the final context. For querying the GroupService the system user context
     * is used.
     *
     * If no result was found, a context with Role NO_ACCESS is returned.
     *
    ...
    public static IAuthorizationContext getAuthorizationContext(UserId pUserId, GroupId pGroupId) throws AuthorizationException {
        ...
        if (pGroupId == null) {
            pGroupId = new GroupId(Constants.USERS_GROUP_ID);
        }
        ...
        try {
            ...
            //NO ACCESS
            return new AuthorizationContext(pUserId, pGroupId, Role.NO_ACCESS);
        }
    }
