...
public final class ZipUtils {
   ...
   * within the zip file.
   *
   * Due to the fact, that we have only one single
   * base path, pDirectory must start with pBasePath to avoid unexpected zip
   * file entries.
   *
   * @param pDirectory The directory containing the input files
  ...
  /**
   * Zip a single file to target directory. zip file add the '.zip' extension to
   * filename.
   *
  ...
  private static void zip(File[] pFileList, String pBasePath, ZipOutputStream pZipOut) throws IOException {
    ...
    try {
      ...
      for (int i = 0; i < pFileList.length; i++) {
        String entryName = pFileList[i].getPath().replaceAll(Pattern.quote(pBasePath), "");
        if (entryName.startsWith(File.separator)) {
          entryName = entryName.substring(1);
        }
        if (pFileList[i].isDirectory()) {
          ...
          pZipOut.closeEntry();
          File[] fileList = pFileList[i].listFiles();
          ...
          try (FileInputStream in = new FileInputStream(pFileList[i])) {
