 ...
 * @author pasic
 */
public interface DataOrganizer extends IConfigurableAdapter {
    ...
    /**
     * Save the {@link IFileTree} instance. fileTree.getDigitalObjectID() must
     * not be null.
     *
    ...
    /**
     * Get a range of the children nodes for some node which is identified by
     * the node parameter.
     *
     ...
     *
     * @throws InvalidNodeIdException your nodeId became invalid due to
    ...
    /**
     * Get a transient handle to the root node for a digital object id. Be
     * aware, the handle may become invalid if the tree is modified. Operations
     * on an invalid handle are safe and result in an exception.
     *
     * @param digitalObjectId The object id.
     *
     * @return The root node id.
     *
     * @throws EntityNotFoundException If no root node was found for the
     * provided object id.
     */
     ...
     * Attributes not present in <i>newData</i> will be deleted, and the new
     * ones will be added.
     *
     * If newData is a collection node also only the attributes are updated.
     * Child nodes won't be synchronized. Of course <i>newData</i>
     * needs to be the a same type of node as the node addressed by nodeId. If
