...
public final class Util {
  ...
  /**
   * Transform the provided data organization node to a JSON representation. If
   * pNode is a CollectionNode, the method is called recursively for all of its
   * child nodes. If pNode is a FileNode it is expected that pNode has a nodeId
   * assigned. Otherwise, an IllegalArgumentException will be thrown.
   *
  ...
  /**
   * Transform the provided JSON object back to a DataOrganization node. For the
   * transformation there are some rules. If pObject is of type FileNode it must
   * provide a nodeId referring to an existing file node in the 'default' view
   * of the DataOrganization of the object with the object id 'oid'. The content
   * of this node (LFN, name and optionally the attributes) are copied to the
   * result node. If pObject is of type CollectionNode there may or may not be a
   * nodeId provided. If a nodeId is provided it has to refer to abn existing
   * collection node. The content of this node is then copied to the result
   * node, including name, all children and optionally the attributes. If
   * pObject provides additional children they are added afterwards in addition
   * to the existing children to the result node.
   *
   ...
   * @param preserveAttributes If TRUE all attributes of an existing nodes will
   * be copied to the resulting node. Otherwise, the resulting node will just
   * have the attributes assigned that are defined in pObject.
   * @param pStrict If TRUE it will be checked that nodes referenced by id have
   * the same type as defined in the JSONObject. Furthermore, it will be checked
   * that file nodes are provided by reference only. If FALSE, also file nodes
   * with custom LFNs may be provided. (default: true)
   *
  ...
  private static IDataOrganizationNode jsonToNode(JSONObject pObject, DigitalObjectId oid, boolean preserveAttributes, boolean pStrict) throws InvalidNodeIdException {
    ...
    System.out.println("ORG " + org.getClass());
