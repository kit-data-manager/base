...
public final class DataOrganizationServiceLocal implements
    ...
    public IFileTree loadFileTree(DigitalObjectId digitalObjectId,
        ...
        IFileTree result = DataOrganizerFactory.getInstance().getDataOrganizer().loadFileTree(digitalObjectId, viewName);
        if (result == null) {
            //no file tree found for the provided object ID
            throw new EntityNotFoundException("No data organization tree was found for digital object '" + digitalObjectId + "'");
        }
        return result;
    }
