...
public class DataOrganizationNode implements IDataOrganizationNode, Serializable,
   ...
   * relationships (parent, children). Also the digitalObjectId is not copied as
   * there is no guarantee that there is this id for the provided node.
   *
  ...
  public void setAttributes(Set<? extends IAttribute> attributes) {
    ...
    for (IAttribute iattr : attributes) {
      myAttributes.add(new Attribute(iattr));
    }
  }
   ...
   * performed before arrived (1.time) to this node during a modified pre-order
   * walk of the tree.
   *
...
//        don.digitalObjectID.setId(digitalObjectID.getId());
  ...
  public NodeId getTransientNodeId() {
    DigitalObjectId digitalObjectID = new DigitalObjectId(digitalObjectIDStr);
