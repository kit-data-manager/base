...
final public class PersistenceFacade {
    ...
    /**
     * Deletes all nodes belonging to a view of for a tree from db. If viewName
     * is null, all views are deleted.
     *
     ...
     *
     * @param relativeRoot The node for which the child count should be
     * obtained.
     *
     ...
     *
     * @param relativeRoot The node for which the children should be obtained.
     * @param relativeDepth The depth. The maximum value is Integer.MAX_VALUE -
     * relativeRoot.getNodeDepth(). If the depth is/gets negative it is set to
     * Integer.MAX_VALUE
     *
    ...
    public List<DataOrganizationNode> getAllNodes(NodeId relativeRoot,
        ...
        query = cb.createQuery(DataOrganizationNode.class);
        query.select(from);
        int depth = rrNode.getNodeDepth() + relativeDepth;
        depth = (depth > 0) ? depth : Integer.MAX_VALUE;
        ...
        query.where(
                cb.and(
                        ...
                        cb.le(from.<Long>get(COL_STEP_NO_LEAVED), rrNode.
                                getStepNoLeaved()),
                        cb.le(from.<Integer>get(COL_NODE_DEPTH), depth)));
    ...
    public List<String> getViewsForDoid(DigitalObjectId doid) {
        ...
        Query q = em.createQuery(
                "SELECT DISTINCT d.viewName FROM DataOrganizationNode d WHERE d.digitalObjectIDStr=?1");
        q.setParameter(1, doid.getStringRepresentation());
