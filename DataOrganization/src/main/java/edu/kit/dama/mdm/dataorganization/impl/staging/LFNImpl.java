...
public final class LFNImpl extends ILFN {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(LFNImpl.class);
    @XmlElement(name = "stringRepresentation")
    ...
    public void fromString(String string) {
        try {
            LOGGER.debug("Parsing LFN URL from string {}", string);
            url = new URL(string);
            LOGGER.debug("Parsed LFN URL: {}", url);
        } catch (MalformedURLException mue) {
            LOGGER.error("Failed to parse LFN URL from string " + string, mue);
