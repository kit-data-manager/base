...
public class GenericExecutionEnvironment {
     ...
     * not (long) capable of processing the task or if its setup fails.
     * Otherwise, a new processing phase of the task will be triggered. Due to
     * the nature of a DataWorkflow task execution there are different possible
     * results:
     *
     * <ul>
     ...
     * unchanged status is returned.</li>
     * <li>The task is in any other intermediate state: The task is executed
     * using the configured environment handler and the current status is
     * returned if the current processing phase has succeeded without error. If
     * the phase could not succeed an exception is thrown.</li>
     * </ul>
     *
     * For details about the different phases please refer to {@link edu.kit.dama.dataworkflow.AbstractExecutionEnvironmentHandler#executeTask(edu.kit.dama.mdm.dataworkflow.DataWorkflowTask)
     * }
     * For the processing workflow this means that
     * {@link #processTask(edu.kit.dama.mdm.dataworkflow.DataWorkflowTask) }
     * should be executed multiple times as long as the result indicated a final
     * state (error or success).The returned status should be checked against
     * DataWorkflowTask.TASK_STATUS.isError() and
     * DataWorkflowTask.TASK_STATUS.isFinished() for failure or success.
     *
     ...
     * configured.
     * @throws DataWorkflowException If the current processing step has failed.
     * The task status should reflect which step exactly has failed.
     * @throws UnsupportedTaskException If the provided task cannot be handled
     * by the environment configured for this instance.
     */
