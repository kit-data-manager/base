...
cp -R assembly/package/tomcat-ext/tomcat-ext-1.1.2.jar assembly/update/tomcat-ext/
handle_result $? "Failed to copy tomcat-ext-1.1.2.jar"
...
echo Copying updated tomcat-ext
cp -R src/resources/$VERSION_NUMBER/update/tomcat-ext assembly/update/
handle_result $? "Failed to copy updated tomcat-ext"
...
echo Building ZIPs
jar -cfM $VERSION"_Update.zip" README.txt CHANGELOG.txt KITDM tomcat-ext sql
